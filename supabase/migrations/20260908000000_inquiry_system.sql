...
create table public.inquiry_messages (
  ...
  constraint inquiry_messages_provider_account_message_key
    unique (provider, gmail_account_email, provider_message_id),
...
create table public.inquiry_sync_state (
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  constraint inquiry_sync_state_pkey primary key (provider, gmail_account_email),
...
begin
  ...
  returning sync.provider, sync.gmail_account_email, sync.gmail_history_id,
    sync.lease_token, sync.lease_expires_at;
end;
$$;
...
begin
  ...
  where d.id = p_draft_id
    ...
    and length(trim(d.gmail_thread_id)) > 0
  ...
  where i.id = v_draft.inquiry_id
    and i.status not in ('converted', 'lost', 'ignored')
  for update of i;
  ...
  set state = 'sending',
      ...
      send_claim_expires_at = clock_timestamp() + interval '15 minutes',
...
begin
  ...
  set state = 'send_failed',
      ...
      send_error = left(coalesce(nullif(trim(p_error), ''), 'Definitive pre-provider send failure'), 1000),
      last_failure_kind = 'pre_provider',
...
begin
  ...
  set state = 'delivery_unknown',
      claim_token = null,
      send_claim_expires_at = null,
      send_error = left(coalesce(nullif(trim(p_error), ''), 'Provider delivery outcome is unknown'), 1000),
      last_failure_kind = 'delivery_unknown',
      failure_recorded_at = clock_timestamp(),
      updated_at = clock_timestamp()
  where d.id = p_draft_id
    and d.state = 'sending'
...
as $$
begin
  if length(trim(coalesce(p_verified_by, ''))) = 0
    or coalesce(p_gmail_evidence ->> 'source', '') <> 'gmail_api'
    or coalesce(p_gmail_evidence ->> 'outcome', '') <> 'not_found'
    or length(trim(coalesce(p_gmail_evidence ->> 'gmail_account_email', ''))) = 0
    or length(trim(coalesce(p_gmail_evidence ->> 'query', ''))) = 0 then
    raise exception 'Verified negative Gmail API evidence and operator identity are required' using errcode = '22023';
  end if;
  ...
  set state = 'send_failed',
      claim_token = null,
      send_claim_expires_at = null,
      reconciled_by = trim(p_verified_by),
      reconciled_at = clock_timestamp(),
      reconciliation_evidence = p_gmail_evidence,
      send_error = 'Verified by Gmail API as not delivered; operator may authorize retry',
      updated_at = clock_timestamp()
  where d.id = p_draft_id
    and (
      d.state = 'delivery_unknown'
      or (d.state = 'sending' and d.send_claim_expires_at <= clock_timestamp())
    )
    and lower(p_gmail_evidence ->> 'gmail_account_email') = d.gmail_account_email
  returning d.id, d.inquiry_id, d.state::text;
end;
$$;
...
as $$
begin
  if length(trim(coalesce(p_authorized_by, ''))) = 0
    or length(trim(coalesce(p_reason, ''))) = 0 then
    raise exception 'Retry authorizer and reason are required' using errcode = '22023';
  end if;

  return query
  update public.inquiry_drafts d
  set state = 'approved',
      retry_authorized_by = trim(p_authorized_by),
      retry_authorized_at = clock_timestamp(),
      retry_reason = left(trim(p_reason), 1000),
      updated_at = clock_timestamp()
  where d.state = 'send_failed'
    and d.id = p_draft_id
  returning d.id, d.inquiry_id, d.state::text;
end;
$$;
...
declare
  ...
  v_outbound_headers jsonb;
begin
  if length(v_provider) = 0
  ...
  if p_reconcile and (
    length(trim(coalesce(p_verified_by, ''))) = 0
    or coalesce(p_gmail_evidence ->> 'source', '') <> 'gmail_api'
    or lower(coalesce(p_gmail_evidence ->> 'gmail_account_email', '')) <> v_draft.gmail_account_email
    or coalesce(p_gmail_evidence ->> 'gmail_message_id', '') <> p_provider_message_id
    or coalesce(p_gmail_evidence ->> 'gmail_thread_id', '') <> p_provider_thread_id
  ) then
    raise exception 'Verified Gmail evidence does not match the draft and provider identifiers' using errcode = '23514';
  end if;

  v_outbound_headers := jsonb_build_object(
    'in-reply-to', v_draft.in_reply_to,
    'references', v_draft.reference_message_ids
  ) || case when p_reconcile then jsonb_build_object(
    'reconciliation', jsonb_build_object(
      'verified_by', trim(p_verified_by),
      'verified_at', v_now,
      'gmail_evidence', p_gmail_evidence
    )
  ) else '{}'::jsonb end;

  if v_draft.state = 'sent' then
    if v_draft.provider is distinct from v_provider
      or v_draft.provider_message_id is distinct from p_provider_message_id
      or v_draft.gmail_message_id is distinct from p_provider_message_id
      or v_draft.gmail_thread_id is distinct from p_provider_thread_id
      or v_draft.sent_subject is distinct from p_sent_subject
      or v_draft.sent_body_text is distinct from p_sent_body then
      raise exception 'Conflicting provider evidence for finalized inquiry draft' using errcode = '23514';
    end if;

    if not exists (
      select 1 from public.inquiry_messages m
      where m.provider = v_provider
        and m.gmail_account_email = v_draft.gmail_account_email
        and m.provider_message_id = p_provider_message_id
        and m.inquiry_id = v_draft.inquiry_id
        and m.direction = 'outbound'
        and m.gmail_thread_id = p_provider_thread_id
        and m.subject = p_sent_subject
        and m.body_text = p_sent_body
        and (not p_reconcile or m.raw_headers -> 'reconciliation' -> 'gmail_evidence' = p_gmail_evidence)
    ) then
      raise exception 'Finalized draft is missing matching outbound evidence' using errcode = '23514';
    end if;

    return query select v_draft.id, v_draft.inquiry_id, 'sent'::text;
    return;
  end if;

  if p_reconcile then
    if not (
      v_draft.state = 'delivery_unknown'
      or (v_draft.state = 'sending' and v_draft.send_claim_expires_at <= clock_timestamp())
    ) then
      raise exception 'Only unknown delivery or an expired stranded claim can be reconciled' using errcode = '55000';
    end if;

  elsif v_draft.state <> 'sending'
    or v_draft.claim_token is distinct from p_claim_token then
    raise exception 'Active send claim is not owned by this worker' using errcode = '55000';
  end if;

  if v_draft.sent_subject is distinct from p_sent_subject
    ...
    raise exception 'Conflicting provider evidence for inquiry draft' using errcode = '23514';
  end if;
  ...
  insert into public.inquiry_messages (
    ...
    p_provider_message_id,
    null,
    v_provider || ':' || v_draft.gmail_account_email || ':' || p_provider_message_id,
    v_draft.gmail_account_email,
    v_draft.to_emails,
    v_draft.cc_emails,
    p_sent_subject,
  ...
  if not found
    or v_message.inquiry_id is distinct from v_draft.inquiry_id
    or v_message.direction is distinct from 'outbound'
    or v_message.gmail_message_id is distinct from p_provider_message_id
    or v_message.gmail_thread_id is distinct from p_provider_thread_id
    or v_message.from_email is distinct from v_draft.gmail_account_email
    or v_message.to_emails is distinct from v_draft.to_emails
    or v_message.cc_emails is distinct from v_draft.cc_emails
    or v_message.subject is distinct from p_sent_subject
    or v_message.body_text is distinct from p_sent_body
    or (p_reconcile and v_message.raw_headers is distinct from v_outbound_headers) then
    raise exception 'Conflicting provider evidence already exists' using errcode = '23514';
  end if;
  ...
  set state = 'sent',
      ...
      sent_at = v_now,
      send_error = null,
      reconciled_by = case when p_reconcile then coalesce(d.reconciled_by, trim(p_verified_by)) else d.reconciled_by end,
      reconciled_at = case when p_reconcile then coalesce(d.reconciled_at, v_now) else d.reconciled_at end,
      reconciliation_evidence = case when p_reconcile then coalesce(d.reconciliation_evidence, p_gmail_evidence) else d.reconciliation_evidence end,
      updated_at = v_now
  where d.id = v_draft.id;

  update public.inquiries i
  set status = case
        when i.status in ('new', 'needs_review', 'drafted') then 'contacted'
        else i.status
      end,
      first_outbound_at = coalesce(i.first_outbound_at, v_now),
      last_outbound_at = greatest(coalesce(i.last_outbound_at, v_now), v_now),
      next_follow_up_at = case
        when i.status in ('converted', 'lost', 'ignored') then null
        else coalesce(i.next_follow_up_at, v_now + interval '3 days')
      end,
      updated_at = v_now
...
set search_path = ''
as $$
  select * from public.complete_inquiry_draft_send(
    p_draft_id, p_claim_token, p_provider, p_provider_message_id,
    p_provider_thread_id, p_sent_subject, p_sent_body, null, null, false
  );
$$;
...
as $$
  select * from public.complete_inquiry_draft_send(
    p_draft_id, null, p_provider, p_provider_message_id,
    p_provider_thread_id, p_sent_subject, p_sent_body,
    p_verified_by, p_gmail_evidence, true
  );
$$;
...
revoke all on function public.require_inquiry_sync_lease(text, text, uuid) from public, anon, authenticated, service_role;
revoke all on function public.complete_inquiry_draft_send(uuid, uuid, text, text, text, text, text, text, jsonb, boolean) from public, anon, authenticated, service_role;
...
revoke all on function public.renew_inquiry_sync(text, text, uuid, integer) from public, anon, authenticated;
grant execute on function public.renew_inquiry_sync(text, text, uuid, integer) to service_role;
...
revoke all on function public.record_inquiry_draft_delivery_unknown(uuid, uuid, text, text, text) from public, anon, authenticated;
grant execute on function public.record_inquiry_draft_delivery_unknown(uuid, uuid, text, text, text) to service_role;
revoke all on function public.reconcile_inquiry_draft_not_delivered(uuid, text, jsonb) from public, anon, authenticated;
grant execute on function public.reconcile_inquiry_draft_not_delivered(uuid, text, jsonb) to service_role;
revoke all on function public.authorize_inquiry_draft_retry(uuid, text, text) from public, anon, authenticated;
grant execute on function public.authorize_inquiry_draft_retry(uuid, text, text) to service_role;
...
revoke all on function public.reconcile_inquiry_draft_send(uuid, text, text, text, text, text, text, jsonb) from public, anon, authenticated;
grant execute on function public.reconcile_inquiry_draft_send(uuid, text, text, text, text, text, text, jsonb) to service_role;
...
-- Sync state can only be mutated through lease-aware RPCs.
