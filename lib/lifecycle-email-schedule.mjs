...
export function parseTourStartDate(value, referenceDate = new Date()) {
  if (!value) return null;
  const monthFirst = value.match(/\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2})(?:st|nd|rd|th)?(?:\s*[–-]\s*(?:[A-Za-z]+\s+)?\d{1,2}(?:st|nd|rd|th)?)?(?:,?\s*(\d{4}))?/i);
  const dayFirst = value.match(/\b(\d{1,2})(?:st|nd|rd|th)?\s+(January|February|March|April|May|June|July|August|September|October|November|December)(?:\s*[–-]\s*\d{1,2}(?:st|nd|rd|th)?\s+[A-Za-z]+)?(?:,?\s*(\d{4}))?/i);
